...
The modularity $Q$ denotes the frequency of edges within a community in respect to the expected number of edges in random graph. It can be masured as\cite{novak2010graph}:
...
\todo{null model purpose (part of intro maybe)}

Suppose, that in a gene family the number of genes is given by discrete random variable $X(t)$. Then probability that $X(t) = c$, given that $X(0) = s$ is $P(X(t) = c | X(0) = s)$.
...
where $\alpha = \frac{\lambda t}{1+ \lambda t}$ and $s \geq 1$. The mean and variation are\cite{hahn2005estimating}:
\begin{align*}
...
One method of identification is to try to delete every branch (one at a time) and recompute the $P$-values. If the $P$-value increases largely, it means that the other branches followed the birth-dead model.
...
\todo{spomenut nejako overall pristup, ze nemozme dat ziadny prior pre root a ze model ma spolocne $\lambda$ pre vsetky hrany a podobne}
