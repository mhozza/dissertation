...
\todo{nejaka historia clankov na tuto temu}
...
Real genomes often contain a lot of repeats (\todo{priklad, niekde bola tusim kvasinka 30\%}) and the sequencing machines produce a lot of errors (\todo{priklad, mozno ten isty co bude v predoslej kapitole}). To provide a good genome size estimate for real data, the estimators have to consider these issues. Let us deal with handling repeats first.
...
% \beta_i & = \frac{\displaystyle\sum_w \Pr(w \in F_i | n(w))}{\displaystyle\sum_{j=1}^{n_f}\sum_w \Pr(w \in F_j | n(w))},\\[1.5ex]
% \Pr(w \in F_i | n(w)) & = \frac{\beta_i}{\displaystyle\sum_{j=1}^{n_f} \alpha_j {\left(\frac{a_j}{a_i}\right)}^{n(w)} e^{(\beta_i-a_j)c_k}}.
% \end{align*}
%
% Note that this method uses standard Poisson distribution for their estimates. Thus we need to calculate the number of $k$-mers which are not present in the data. We can use following formula to estimate it~\cite{waterman}:
% $$H_k[0] = \frac{\displaystyle\sum_{i=1}^{n_f}\beta_i e^{-a_i c_k} \sum_{i=1}^{|H_k|}H_k[i]}{\displaystyle\sum_{i=1}^{n_f}\beta_i\left(1 - e^{-a_i c_k}\right)}$$
...
\paragraph{}The KSA method~\cite{williams} was tested only on real data --- multiple strains of \emph{E. coli}. The genome size results were validate by experimental evaluation (pulsed-field gel electrophoresis, PFGE). The coverage ranged from $55\times$ (strain B) to $85\times$ (strain A). The comparison of the genome sizes obtained by PFGE and by $21$-mer analysis is in table~\ref{tab:williams}. \todo{2 vety aj o repeatoch}
\begin{table}[htbp]
\centering
...
We also compared our method to KSA~\cite{williams} on both simulated (Fig.~\ref{fig:covest-ksa-sim}) and real data (Table~\ref{tab:covest-ksa-ecoli}).
...
The second approach, which we presented in CovEst~\cite{covest}, works well even in very low coverage data ($0.5\times$) or with highly erroneous data ($10\%$ error rate). Our approach is also very universal and easily extensible to more complex models.
